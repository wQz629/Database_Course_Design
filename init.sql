...
DROP TABLE IF EXISTS inventory_return;   -- 若你后面要补“库存退货”，先占位可删
...
-- （可选占位）库存退货表：你截图里只写了表名没给字段。
-- 如果你不需要库存退货功能，删掉这一段即可。
CREATE TABLE inventory_return (
  inv_return_id  CHAR(10)      NOT NULL COMMENT '库存退货单号',
  medicine_id    CHAR(10)      NOT NULL COMMENT '药品ID',
  emp_id         CHAR(5)       NOT NULL COMMENT '操作员ID',
  ...
  PRIMARY KEY (inv_return_id),
  CONSTRAINT fk_ir_medicine FOREIGN KEY (medicine_id)
    REFERENCES medicine(medicine_id)
    ON UPDATE CASCADE ON DELETE RESTRICT,
  CONSTRAINT fk_ir_employee FOREIGN KEY (emp_id)
    REFERENCES employee(emp_id)
    ON UPDATE CASCADE ON DELETE RESTRICT,
